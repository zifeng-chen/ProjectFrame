...
class WRCustomNavigationBar: UIView
{
    ...
    fileprivate lazy var txtView:UIView = {
        let view = UIView()
        view.isHidden = true
        return view
    }()
...
//        view.backgroundColor = UIColor(red: (218.0/255.0), green: (218.0/255.0), blue: (218.0/255.0), alpha: 1.0)
    ...
    func updateFrame()
    {
        ...
        leftButton.frame = CGRect(x: 8, y: top, width: buttonWidth, height: buttonHeight)
        rightButton.frame = CGRect(x: WRScreenWidth-buttonWidth-margin-10, y: top, width: buttonWidth, height: buttonHeight)
        titleLabel.frame = CGRect(x: (WRScreenWidth-titleLabelWidth)/2.0, y: top, width: titleLabelWidth, height: titleLabelHeight)
//        txtView.frame = CGRect(x: 80, y: top, width: 210, height: titleLabelHeight)
        txtView.snp.makeConstraints { (make) in
            make.top.equalTo(top)
            make.width.equalTo(WRScreenWidth-160)
...
extension WRCustomNavigationBar
{
    ...
    func wr_setLeftButton(normal:UIImage, highlighted:UIImage) {
        wr_setLeftButton(normal: normal, highlighted: highlighted, title: nil, titleColor: nil, font:nil)
    }
    func wr_setLeftButton(image:UIImage) {
        wr_setLeftButton(normal: image, highlighted: image, title: nil, titleColor: nil, font:nil)
    }
    func wr_setLeftButton(title:String, titleColor:UIColor,font:UIFont) {
        wr_setLeftButton(normal: nil, highlighted: nil, title: title, titleColor: titleColor, font:font)
    }
    ...
    // 左右按钮私有方法
    private func wr_setLeftButton(normal:UIImage?, highlighted:UIImage?, title:String?, titleColor:UIColor?,font:UIFont?) {
